// Vendored from the espressif/es8311 component, version 1.0.0, with one
// change: its two I2C calls go through the board bus instead of the old
// i2c driver the component was written against. The old driver and the
// i2c_master one jdBasic uses cannot live in the same binary - IDF
// aborts at boot with "CONFLICT! driver_ng is not allowed to be used
// with this old driver" - and the coefficient table below is the last
// thing worth retyping, so the file moves rather than the values.
/*
...
extern esp_err_t es3c28p_i2c_write(int addr, const uint8_t* data, int n);
extern esp_err_t es3c28p_i2c_read(int addr, uint8_t reg, uint8_t* out, int n);
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return es3c28p_i2c_write(es->dev_addr, write_buf, 2);
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return es3c28p_i2c_read(es->dev_addr, reg_addr, reg_value, 1);
}
