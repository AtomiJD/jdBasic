...
* **`form`**: `name` (event prefix), `title`, `width`/`height` (client area). Optional: `"mdi": true` makes it an MDI frame; `"menu": [...]` is a `FORM.MENU` spec and `"toolbar": [...]` a `FORM.TOOLBAR` spec, both built at load time (handlers auto-bind like everything else).
