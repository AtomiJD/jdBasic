...
| `fake.jdb` | Faker | made up names, addresses, companies, account numbers, dates and text in English and German, from a seeded generator that answers the same on every machine or, with SEEDRNG, from the RNG builtins, plus a table built from a spec | [fake_lib_readme.md](fake_lib_readme.md) |
...
| `jdweb.jdb` | Flask, the Werkzeug test client, Flask-WTF CSRF | web apps on HTTP.SERVER: routes with path parameters for any method, middleware before and after, sessions with idle and absolute expiry, id rotation, a SQLite store, CSRF protection and flash messages, a cookie builder, static files, JSON and error responses, a test client for the running app, and the themed page chrome and cookie login jdTrakr runs on | [jdweb_lib_readme.md](jdweb_lib_readme.md) |
...
| `svg.jdb` | svgwrite, pygal, matplotlib (basic charts) | SVG without a window: drawings with shapes, paths, text, groups, transforms and gradients; line, bar, stacked bar, scatter, pie, area, histogram and box charts with axes, ticks, a legend and colours for light and dark pages, straight from DF frames; lines along numbers, log scales, reference lines, notes and fitted curves; box statistics as matplotlib computes them; sparklines | [svg_lib_readme.md](svg_lib_readme.md) |
...
| `secret.jdb` | passlib, Django's password hashers, secrets, hmac.compare_digest | password hashes with PBKDF2-HMAC-SHA256 at 600000 rounds that also read Django's format, rehash decisions, comparison in constant time, random tokens in base64url or hex, API keys kept as hashes; checked against Python hashlib | [secret_lib_readme.md](secret_lib_readme.md) |
| `fit.jdb` | numpy.linalg.lstsq, numpy.polyfit, scipy.optimize.curve_fit, scipy.interpolate.CubicSpline, scipy.optimize root finders | least squares with the minimum-norm answer and the pseudo-inverse, polynomial fits with weights, Levenberg-Marquardt curve fits with parameter errors and covariance, linear interpolation and cubic splines (not-a-knot, natural, clamped) with derivatives, Brent, bisection, secant and Newton roots, a bounded Brent minimum; checked against numpy and scipy | [fit_lib_readme.md](fit_lib_readme.md) |
| `qr.jdb` | segno, qrcode, segno.helpers.make_epc_qr | QR codes in jdBasic: numeric, alphanumeric and UTF-8 byte mode, levels L, M, Q and H, versions 1 to 40, Reed-Solomon over GF(256), the eight masks scored by the ISO penalty rules; the EPC payment code (GiroCode) checked with VALID and MONEY; output as terminal text, SVG or boxes on a PDFGEN page; matrices equal to segno | [qr_lib_readme.md](qr_lib_readme.md) |
| `metrics.jdb` | prometheus_client | counters, gauges and histograms with labels in one registry, the Prometheus text format 0.0.4 byte for byte as the Python client writes it, a timer, and JDWEB middleware counting requests and latency per route pattern with a /metrics page | [metrics_lib_readme.md](metrics_lib_readme.md) |
| `search.jdb` | rank_bm25, Whoosh (analysis and BM25) | full-text search in memory: an inverted index with positions, case and accent folding, English and German stop words and light stemming, BM25 ranking equal to rank_bm25's BM25Okapi, field weights, phrase queries, typo-tolerant terms through a trigram index and FUZZY, did-you-mean suggestions, index files and reciprocal rank fusion | [search_lib_readme.md](search_lib_readme.md) |
| `i18n.jdb` | gettext, babel (plural rules, locale negotiation), python-i18n | texts per locale from JSON, YAML, TOML or INI with a de-AT -> de -> en fallback chain and {name} placeholders, CLDR plural rules for en, de, fr and pl with an engine for more, Accept-Language parsing and negotiation for JDWEB, a current locale for NUMFMT, DT and the TMPL filters t, number and currency; checked against babel | [i18n_lib_readme.md](i18n_lib_readme.md) |
| `archive.jdb` | tarfile, gzip, shutil.make_archive | tar read and write in ustar, GNU and pax with long names, modes, mtimes, owners and links; gzip with names, mtimes and several members; a folder to .tar.gz with filters and fixed mtimes, and extraction that refuses paths leaving the target; archives open in bsdtar and Python and theirs open here | [archive_lib_readme.md](archive_lib_readme.md) |
| `rand.jdb` | numpy.random, random | seeded generators on the RNG builtins; uniform, normal, lognormal, exponential, triangular, gamma, beta, Poisson and binomial draws; weighted choice by running sum or alias table; sampling with and without replacement, shuffle, permutation and reservoirs, the same in both backends; checked against scipy.stats | [rand_lib_readme.md](rand_lib_readme.md) |
| `struct.jdb` | struct, ctypes structures, construct | binary records declared once: fields with type, count and byte order, nested layouts and arrays, bit fields in ctypes order, fixed texts, magic values, offsets; read into a map and written back, CRC-16/MODBUS and a hex dump | [struct_lib_readme.md](struct_lib_readme.md) |
| `img.jdb` | Pillow (PNG, crop, resize, transpose, convert L, ImageFilter.Kernel, alpha_composite) | images as pixel arrays without GFX: PNG read in every bit depth and color type with palettes, tRNS, Adam7 and CRC checks, PNG write with all five filters, an adaptive choice, deflate level and palette output; crop, nearest and bilinear resize, quarter turns, flips, grayscale, 3x3 and 5x5 kernels, median-cut and web palettes, paste, alpha compositing and data URIs, pixel-exact against Pillow | [img_lib_readme.md](img_lib_readme.md) |
| `pool.jdb` | concurrent.futures, multiprocessing.Pool, asyncio.gather, aiolimiter | worker pools over ASYNC tasks: results in input order, errors per item with fail fast, per-job timeouts that drop late answers, cancellation, WAITALL and WAITANY, a token-bucket rate limiter shared by workers, and a fan-in over several channels | [pool_lib_readme.md](pool_lib_readme.md) |
