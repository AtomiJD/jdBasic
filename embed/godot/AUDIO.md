# GODOT.AUDIO - sound and music for jdBasic-in-Godot
...
|----------|---------|-------|
| `GODOT.AUDIO.PLAY(path [, volume_db [, pitch]])` | handle | Fire-and-forget SFX; the player frees itself when the sound finishes |
| `GODOT.AUDIO.MUSIC(path [, volume_db])` | handle | Looping music on one reusable player; calling it again swaps the track |
| `GODOT.AUDIO.STOP_MUSIC()` | - | Stop and free the music player |
| `GODOT.AUDIO.STOP(handle)` | bool | Stop and free any player a PLAY/MUSIC call returned |
...
`GODOT.AUDIO.PLAY` calls (footsteps, shots) don't pile up.
...
```basic
SUB on_hit()
    GODOT.AUDIO.PLAY("res://sfx/hit.wav", -3.0)
ENDSUB

SUB _ready()
    self_h = GODOT.SELF()
    GODOT.AUDIO.MUSIC("res://music/theme.ogg", -8.0)   ' starts looping
ENDSUB

SUB on_game_over()
    GODOT.AUDIO.STOP_MUSIC()
    GODOT.AUDIO.PLAY("res://sfx/lose.wav")
ENDSUB
```
